...
import com.g5.form.MuaSanPhamJDialog;
...
public class ItemSanPham extends javax.swing.JPanel {
    ...
    public void setThongTin(String tensp, String gia, String giakm, String loai) {
        this.tensp = tensp;
        String giaMD = gia;

        String giakmGoc = Validate.chuyenGiaMacDinh(giakm);

        float giaKMMD = Float.parseFloat(Validate.chuyenGiaMacDinh(giakm));
        if (giaKMMD != -1) {
            giaMD = ItemSanPham.addGachNgang(giaMD);
        } else {

        }
        this.thongtin = tensp.trim() + "\n\n" + giaMD.trim() + "\n" + (giakmGoc.trim().equalsIgnoreCase("-1") ? ItemSanPham.addGachNgang(" ") : giakm.trim()) + "\n" + loai.trim();
...
//                MuaSanPhamJDialog.loadSoLuong();
    ...
    public static String addGachChan(String input) {
        StringBuilder result = new StringBuilder();

        // Duyệt qua từng ký tự trong chuỗi
        for (int i = 0; i < input.length(); i++) {
            // Thêm ký tự vào chuỗi kết quả và ký tự gạch chân Unicode
    ...
    private void initComponents() {
        ...
        txtThongTin.setFont(new java.awt.Font("Tahoma", 0, 13)); // NOI18N
        txtThongTin.setRows(5);
        txtThongTin.setText("Cà phê\n10,000 VND\n8,000 VND\nCà phê\n");
        txtThongTin.setAutoscrolls(false);
        txtThongTin.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 0, 0)));
        txtThongTin.setFocusable(false);
        txtThongTin.setMargin(new java.awt.Insets(0, 0, 0, 0));
