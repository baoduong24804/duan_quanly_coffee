...
public class SanPhamDao implements SanPhamDAOinterface {
    ...
    String getSLmua = "select sum(soluong) as sl from HoaDonChiTiet where masp = ?";
    String getTongTien = "select sum(gia*soluong) as tongtien from HoaDonChiTiet where mahd = ?";

    public float getTongTien(int mahd) {
        ResultSet rs = null;
        float sl = 0;
        try {
            rs = JDBCHelper.executeQuery(getTongTien, mahd);
            rs.next();
    ...
    public int getSLmua(int masp) {
        ResultSet rs = null;
        int sl = 0;
        try {
            rs = JDBCHelper.executeQuery(getSLmua, masp);
            rs.next();
            sl = rs.getInt("sl");
            return sl;
        } catch (Exception e) {
    ...
    public int getSoLuongSP(int masp) {
        ...
        int sl = 0;
        try {
            rs = JDBCHelper.executeQuery(getSoLuongSP, masp);
            rs.next();
    ...
    public void upSoluongBiTrung(int soluong, int mahd, int masp, String size) {
        int sl = 0;
        sl = getSL(masp, size, mahd) + soluong;
