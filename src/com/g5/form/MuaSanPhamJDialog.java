...
import com.g5.component.ItemSanPham;
...
import com.g5.component.PanelSanPham;
...
public class MuaSanPhamJDialog extends javax.swing.JDialog {
    ...
    public static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(MuaSanPhamJDialog.class);
    ...
    void FillTableHoaDonChiTet(int MaHD) {
        ...
        for (HoaDonChiTiet hd : list2) {// danh sách các sản phẩm trong giỏ hàng
            ...
            modelTblHDCT.addRow(new Object[]{hd.getMaHDCT(), hd.getMaSP(), sp.getTenSP(), soluong, hd.getSize(), Validate.chuyenGiaVietNam(gia), Validate.chuyenGiaVietNam(tt)});
                ...
                for (SanPham sanPham : list) {
                    ...
                    if (daoKM.getPTKMcuaSP(sanPham.getMaSP()) != 0) {
                        try {
                            ...
                            //    System.out.println(a+" a "+sanPham.getTenSP());
                            //    System.out.println(gia+" giá "+sanPham.getTenSP());

                            //    System.out.println(giagiam+" giảm "+sanPham.getTenSP());
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    ...
                    if (MuaSanPhamJDialog.class.getResource("/com/g5/image/" + sanPham.getHinh().trim()) != null) {
                        sp = new ItemSanPham(new ImageIcon(MuaSanPhamJDialog.class.getResource("/com/g5/image/" + sanPham.getHinh().trim())));
                    } else {
                        sp = new ItemSanPham(null);
                    }

                    //    sp.setThongTin(sanPham.getTenSP(),  (giagiam != -1 ? ItemSanPham.addGachNgang(Validate.chuyenGiaVietNam(gia))+ "\n" + Validate.chuyenGiaVietNam(giagiam)+"\n" : "\n" + Validate.chuyenGiaVietNam(gia) + "\n") + sanPham.getLoaiSP());
                    sp.setThongTin(sanPham.getTenSP(), Validate.chuyenGiaVietNam(gia), Validate.chuyenGiaVietNam(giagiam), sanPham.getLoaiSP());
                    sp.setGia(gia);
...
//                    }
    ...
    public void loadTongTien() {
        ...
        float tt = 0;
        tt = daoSP.getTongTien(Integer.parseInt(txtMaHD.getText()));
    ...
    public void taiThongTinLenList() {
        ...
        int soluong = daoSP.getSoLuongSP(masp);
        int soluonMua = daoSP.getSLmua(masp);
        int max = soluong - soluonMua;
        if (max < Integer.parseInt(txtSoLuong.getText().trim())) {
    ...
    public void loadCapnhat() {
        ...
        int mahdct = 0;
        mahdct = Integer.parseInt(lblMaHDCT.getText().toString().replace("Mã HDCT:", "").trim());
        ...
        for (int i = 0; i < tblHDCT.getRowCount(); i++) {
            int mahdct2 = Integer.parseInt(tblHDCT.getValueAt(i, 0).toString());
            int masp2 = Integer.parseInt(tblHDCT.getValueAt(i, 1).toString());
            int soluong2 = Integer.parseInt(txtSoLuong.getText());
            //   float gia = Float.parseFloat(Validate.chuyenGiaMacDinh(txtGia.getText()));
            //   float tt = Float.parseFloat(Validate.chuyenGiaMacDinh(txtThanhTien.getText()));
            //   int masp = Integer.parseInt(tblHDCT.getValueAt(i, 1).toString());
            if (masp2 == Integer.parseInt(txtMaSP.getText()) && mahdct2 == mahdct) {
                tblHDCT.setValueAt(soluong2, i, 3);
    ...
    public void capNhatSoLuong() {
        int masp = Integer.parseInt(txtMaSP.getText().trim());
        int soluong2 = daoSP.getSoLuongSP(masp);
        int soluonMua = daoSP.getSLmua(masp);
        int max = soluong2 - soluonMua;
        if (max < Integer.parseInt(txtSoLuong.getText().trim())) {
            TextMes.Alert(null, "Không đủ số lượng");
            return;
        }
        ...
        try {
            daoHDCT.updateSoLuong(soluong, mahdct);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void capNhat() {
        int mahd = Integer.parseInt(txtMaHD.getText());
        String size = cboSize.getSelectedItem().toString().trim();
        int mahdct = 0;
        mahdct = Integer.parseInt(lblMaHDCT.getText().toString().replace("Mã HDCT:", "").trim());
        try {
            daoHDCT.update2(size, mahd, mahdct);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void xacNhan() {
        ...
        for (ItemSanPham item : danhSachSanPham) {
            ...
            if (item.getSelectdSP() && !item.getTongtienMua().trim().equals("0")) {
                ...
                hdct.setSoluong(Integer.parseInt(txtSoLuong.getText().trim()));
                //   System.out.println(item.getSoluongMua());
                ...
                try {
                    if (daoHDCT.getSPbiTrung(hdct.getMaHD(), hdct.getMaSP(), hdct.getSize()) == 1) {/// tim thay sp
                        ...
                        FillTableHoaDonChiTet(hdct.getMaHD());
                        //   return;
                    } else {
                        daoHDCT.create(hdct);
...
//            System.out.println(Validate.chuyenGiaMacDinh(Validate.chuyenGiaVietNam(gia)) +"MD");
    ...
    private void initComponents() {
        ...
        tblHDCT.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã HDCT", "Mã SP", "Tên SP", "Số lượng", "Size", "Giá", "Tổng tiền"
            }
        ) {
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            ...
                            .addComponent(txtTonKho, javax.swing.GroupLayout.DEFAULT_SIZE, 220, Short.MAX_VALUE)
        ...
        jButton4.setText("Cập nhật Size");
        ...
        lblMaHDCT.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        lblMaHDCT.setText("Mã HDCT:");
    ...
    private void tblHDCTMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblHDCTMouseClicked
        ...
        txtTonKho.setText(sp.getSoLuong() - daoSP.getSLmua(masp) + "");
        ...
        lblMaHDCT.setText("Mã HDCT: " + tblHDCT.getValueAt(tblHDCT.getSelectedRow(), 0).toString());
    ...
    public boolean kiemTraSLTrenBang() {
        try {
            for (int i = 0; i < tblMuaHang.getRowCount(); i++) {
                if (Integer.parseInt(tblMuaHang.getValueAt(i, 3).toString().trim()) == 0) {
                    return false;
                }
            }
        } catch (Exception e) {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (!kiemTraSLTrenBang()) {
            TextMes.Alert(null, "Vui lòng chọn số lượng");
            return;
        }
