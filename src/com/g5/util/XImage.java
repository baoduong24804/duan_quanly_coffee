...
public class XImage {
    ...
    public static void selectImage3(String file, JLabel lbl, int width, int height) {
        try {
            //   BufferedImage originalImage = ImageIO.read(new File(file.toString()));
            lbl.setText("");
            ImageIcon icon;

            if (XImage.class.getResource("/com/g5/image/" + file.trim()) == null) {
                icon = new ImageIcon(XImage.class.getResource("/com/g5/image/" + "null.png" ));
            } else {
                icon = new ImageIcon(XImage.class.getResource("/com/g5/image/" + file.trim()));
            }
